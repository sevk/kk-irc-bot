...
class IRC
  def initialize(server,port,nick,channel,charset,name=$name)
    ...
    #当前的状态, 0=>need_conntoserv , 1=>need_identify , 11=>need_ghostnick
    #, 2=>need_join
    @irc_stat = 0 
  ...
  #发送msg消息,随机 delay 秒数.
  #sSay 不能为空
  def msg(who,sSay,delay=nil,flag=0)
  ...
  def check_msg(s)
    ...
    when /^:(.+?)!(.+?)@(.+?)\sPRIVMSG\s(.+?)\s?:(.+)$/i
      ...
      #以我的名字开头
      when /^#{Regexp::escape @nick}[\s,:`]?(.*)$/i 
        s=$1.to_s.strip #消息内容
        case s.force_encoding('utf-8')
        when /源码/
          msg(to, $my_s)
          return
        end
  ...
  def tran_url(url,from,to,force=true)
    ...
    unless force
      ...
      #return if to !~ /#{Regexp.escape(@channel)}/i
    end
    p url
    $last_url = url.clone
    ...
    #@ti_p.kill rescue nil
    @ti_p=Thread.new(to,from,url) { |to,from,url|
      ti = gettitleA(url,from,true)
      if ti
        @ti.kill
        msg(to, ti ,0)
      end
  ...
  def check_irc_event(s)
    ...
    when /^ERROR\s:(.*?):\s(.*?)$/i # Closeing
      ...
      return if @exit
      $need_reconn=true
    when /.+?404\s#{@nick.force_encoding('ascii-8bit') }\s#{@channel}\s:Cannot send to channel/
      puts s
      identify
    else
  ...
  def iSend(s='')
     ...
     when /^\/nick\s+(.*)$/i
       @nick = $1.force_encoding('ascii-8bit')
        send s.gsub(/^[\/]/,'')
